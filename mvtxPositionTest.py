...
def cmmCleanFit(argsfidList, argspointList, argssigmaList):
    ...
    bashCmd = f'make fit && ./RotTransFit {args.input_model} {cmm_file_corrected} {args.r} {args.a} {args.b} {args.g} {args.xt} {args.yt} {args.zt} {args.xt0} > output/fit_results.txt'
    os.system(bashCmd)

    # Read and clean data
    with open('output/fit_results.txt','r', encoding='utf8') as f:
        fit_results = f.read()
    # print(fit_results)
...
def plotResults(fit_results_plane_df, fit_results_cylinder_df, figName=""):
    ...
    fig_residuals_plane.savefig(f'output/residuals_plane_{figName}.pdf')
    fit_results_plane_df['fid'] = fit_results_plane_df['fid'].astype(str)
    # if (2 in fit_results_plane_df['ftype']): return
    ...
    fig_residuals_plane2.write_html(f'output/residuals_plane_{figName}.html')
    ...
    fig_yz.savefig(f'output/yz_{figName}.pdf')
...
plotResults(fit_results_plane_df, fit_results_cylinder_df, os.path.splitext(os.path.basename(args.input_cmm))[0])
